...
export const LoginPage: React.FC = () => {
  ...
  const saveSession = (
    ...
    if (response.user) {
      const backendRole = (
        response.user as typeof response.user & {
          activeRole?: string;
          roles?: string[];
        }
      ).activeRole;
      ...
      const backendRoles =
        (response.user as typeof response.user & { roles?: string[] }).roles ||
        [];
      if (
        backendRoles.includes("VENDOR_OWNER") &&
        backendRoles.includes("BUYER")
      ) {
        role = "student_vendor";
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          phoneNumber:
            ...
            (response.user as { phone?: string | null }).phone ||
            "",
        }),
      );
    }
  ...
  const handleLoginSubmit = async (data: LoginInput) => {
    ...
    setActivePortal(
      role === "vendor" || role === "student_vendor" ? "vendor" : "buyer",
    );
    window.location.href =
      role === "vendor" || role === "student_vendor" ? "/vendor" : "/";
  };
  ...
  const handleRegisterSubmit = async (data: RegisterInput) => {
    ...
    window.location.href = data.role === "vendor" ? "/vendor" : "/";
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] w-full grid grid-cols-1 lg:grid-cols-2 bg-slate-50/80 dark:bg-slate-950 overflow-hidden px-6 lg:px-16 py-8 items-center">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#00000008_1px,transparent_1px),linear-gradient(to_bottom,#00000008_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_70%_60%_at_50%_50%,#000_70%,transparent_100%)] pointer-events-none" />
